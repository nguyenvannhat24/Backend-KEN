...
exports.getKeycloakUserById = async (req, res) => {
  try {
    ...
    if (!user)
      return res
        .status(404)
        .json({ success: false, message: "User not found in Keycloak" });
      ...
      if (typeAccount === "SSO") {
        ...
        const keycloakPayload = {
          ...
          firstName: req.body.full_name
            ? req.body.full_name.split(" ")[0]
            : checkUser.full_name,
          lastName: req.body.full_name
            ? req.body.full_name.split(" ").slice(1).join(" ")
            : "",
...
exports.viewProfile = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ success: false, message: "userId là bắt buộc" });
    }
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (user.password_hash && !current_password) {
      return res
        .status(400)
        .json({ success: false, message: "Cần nhập mật khẩu hiện tại" });
    }
  ...
  try {
    ...
    if (userInDB.typeAccount === "SSO") {
      ...
      const keycloakPayload = {
        ...
        firstName: updatedUser.full_name
          ? updatedUser.full_name.split(" ")[0]
          : userInDB.full_name,
        lastName: updatedUser.full_name
          ? updatedUser.full_name.split(" ").slice(1).join(" ")
          : "",
        attributes: {
