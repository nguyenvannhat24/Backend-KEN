...
class BoardMemberService {
  ...
  async addMember({ requester_id, user_id, board_id, role_in_board }) {
    ...
    const requesterMember = await boardMemberRepo.findMember(
      requester_id,
      board_id
    );
    if (!requesterMember) {
    ...
    const dataNotifile = await this.notification(
      user_id,
      board_id,
      role_in_board,
      "add_member"
  ...
  async updateRole(requester_id, user_id, board_id, role_in_board) {
    ...
    const isRequesterCreator = await boardRepo.isCreatorFromMember(
      requester_id,
      board_id
    );
    ...
    // Thực hiện cập nhật
    const member = await boardMemberRepo.updateRole(
      user_id,
      board_id,
      role_in_board
    );
  ...
  async removeMember(requester_id, user_id, board_id) {
    ...
    if (requester_id.toString() !== user_id) {
      ...
      const isCreator = await boardRepo.isCreatorFromMember(
        requester_id,
        board_id
      );
      const requester = await userRepo.findById(requester_id);
      if (!requester)
        throw new Error(
          "Chỉ người tạo hoặc admin mới có quyền xóa thành viên khác"
        );
